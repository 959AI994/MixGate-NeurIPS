...
class Model(nn.Module):
    ...
    def forward(self, G):
        ...
        if self.enable_encode:
            ...
            hs = generate_hs_init(G, hs, self.dim_hidden, mig=True)  
        else:
            hs = torch.zeros(num_nodes, self.dim_hidden)
        ...
        not_mask = G.mig_gate.squeeze(1) == 2  
        and_mask = G.mig_gate.squeeze(1) == 3  
        or_mask = G.mig_gate.squeeze(1) == 4   
        maj_mask = G.mig_gate.squeeze(1) == 1  
        xor_mask = G.mig_gate.squeeze(1) == 5  
